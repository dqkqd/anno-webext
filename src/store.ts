...
export async function getStoredAnnotations<Meta>(): Promise<StoredAnnotations<Meta>> {
...
export async function readAll<M, S>(decodeMetadata: (s: S) => M): Promise<Annotations<M>> {
	const result = await chrome.storage.local.get({ highlights: {} });
